...
class NewsByCategory {
  ...
  NewsByCategory({String tid, String name, List<News> news}) {
    this._tid = tid;
    this._name = name;
    this._news = news;
  }
  ...
  List<News> get items => _news;
  set items(List<News> items) => _news = items;

  NewsByCategory.fromJson(Map<String, dynamic> json) {
    _tid = json['tid'];
    _name = json['name'];
    if (json['items'] != null) {
      _news = new List<News>();
  ...
  Map<String, dynamic> toJson() {
    ...
    if (this._news != null) {
      data['items'] = this._news.map((v) => v.toJson()).toList();
    }
    return data;
  }
}
